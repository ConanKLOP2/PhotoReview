...
public partial class DiagnosticsWindow : Window
{
    public DiagnosticsWindow(ReviewMetricsSnapshot snapshot, ExplorerViewSnapshot? explorerSnapshot = null)
    {
        ...
        CacheHitsText.Text = snapshot.CacheHits.ToString("N0", CultureInfo.CurrentCulture);
        CacheMissesText.Text = snapshot.CacheMisses.ToString("N0", CultureInfo.CurrentCulture);
        HitRateText.Text = total == 0 ? "N/A" : $"{(double)snapshot.CacheHits / total:P1}";
        SourceBytesText.Text = $"{snapshot.SourceBytesRead:N0} bytes";
        SourceReadsText.Text = snapshot.SourceReads.ToString("N0", CultureInfo.CurrentCulture);
