...
- Evidence bổ sung: Diagnostics view đã hiển thị metrics trong phiên hiện tại, source-byte counter loại trừ disk-cache hit, và present-latency trung bình ms/ảnh; chưa thay thế benchmark tự động trên fixture chuẩn.
...
- Evidence: runtime đã đọc các field chính và thêm mapping cấu hình cho Skip/Undo/Fullscreen; click ảnh thường không còn handler điều hướng, compare preview giữ handler chọn riêng. Một số action legacy và keyboard paths vẫn nằm trực tiếp trong `MainWindow`.
...
- Evidence: `ImageSortService.Sort` gọi decoder cho từng file trước khi hiển thị folder; logic đã tách khỏi code-behind, có test hành vi natural numeric ordering và fallback metadata; fixture EXIF thật vẫn còn thiếu.
