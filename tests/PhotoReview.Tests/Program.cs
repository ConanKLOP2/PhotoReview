...
try
{
    ...
    Check(File.Exists(Path.Combine(projectRoot, "src", "PhotoReview.Platform.Windows", "WindowsRecycleBin.cs")) && (mainWindow.Contains("_recycleBin.TryRestore") || mainWindow.Contains("WindowsRecycleBin")), "Delete Undo restores through Recycle Bin Shell (source presence: restoring needs the real Recycle Bin)", failures);
