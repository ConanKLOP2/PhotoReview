...
- **Cập nhật:** 2026-09-19 | **Branch làm việc:** `refactor/T46a-mainviewmodel-nav` -> `refactor/integration` · VERIFY đạt, xUnit 729/729 (Core: 245, Imaging: 144, Integration: 8, Architecture: 6 (Rule 6 K-2 enabled, 0 skipped), App.Tests: 70, Tests.Unit: 256)
...
1. **Lựa chọn task tiếp theo:**
   - **T45b (`CompareViewModel`):** ViewModel so sánh ảnh song song (phụ thuộc T41a - đã sẵn sàng).
   - **T45c (`ImagePresenter`):** Trình diễn ảnh và điều phối hiển thị (phụ thuộc T44, T45a, T45b, T84).
   - **T46a (`MainViewModel: Mở và điều hướng`):** Chuyển giao các luồng mở thư mục và điều hướng sang VM.
