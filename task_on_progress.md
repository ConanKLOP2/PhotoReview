...
**Updated:** 2026-09-26 (late night) | **Base:** master `f7f6adc` (#94–#107 merged) | **Open:** benchmark speed-up PR

## Now

- **Benchmark speed-up — PR open (this branch `perf/benchmark-speed-rebased`):** `run-matrix.ps1` one warm-up per (fixture, mode, condition) per batch (`-WarmupEveryCell` = old behaviour), `-Profile quick` uses new `s2-next-slow-quick` (40 keys); `BenchmarkWindow` "Quick check" button (fast-sequential, 10 iterations), image limit (default 64, 0 = all), non-blocking cache teardown. F4: gate 393 → 260 s (−34 %), quick 209 → 158 s (−24 %), in-app quick check 2.6 s vs default 8.2 s. **User GUI check:** Quick check button, image-limit box, close the window mid-run (no crash, no leftover `%TEMP%\PhotoReview-Benchmark-Cache\*`).
...
- **Still to do:** **user GUI check for AR13**: pan + glide, wheel zoom at cursor, Ctrl+wheel navigation, click-to-zoom, Fit after resize and DPI change, skipped-files list opens. Also seen once: a `JournalConcurrencyTests` file-lock crash (passed on rerun).
