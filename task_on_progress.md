...
- **Cập nhật:** 2026-09-17 | **Branch làm việc:** `refactor/integration` · VERIFY đạt, xUnit 414/414 (Core: 155/155, Tests.Unit: 259/259)
...
- **Mục tiêu:** Tái cấu trúc B + C3 (`docs/refactoring/REFACTOR-PLAN.md`, `REFACTOR-TASKS.md`) kết hợp chẩn đoán hiệu năng WD (`PERF-DIAGNOSIS-PLAN.md`, `PERF-DIAGNOSIS-TASKS.md`). Wave 2: Hoàn tất nhóm song song ∥D và Task T21b (`AppSettings` dùng enum). Sẵn sàng thực hiện T21c (`JournalEntry` dùng enum) và T24 (chuyển layout `src/`/`tests/`).
...
- **Refactor:** T00, T02, T03, T04, T05, T10, T11, T12, T13a, T13b, T14a, T14b, T14c, T14d, T20, T21a, T22a, T22b, T22c, T23a, T23b, T21b (Hoàn tất 100% nhóm song song ∥D và Task T21b; nhật ký chi tiết trong `REFACTOR-TASKS.md`).
...
1. **Refactor (Wave 2 tiếp tục):**
   - **T21c:** `JournalEntry` dùng enum (`FileOperationType`, `JournalState`) kèm converter và fixture `operations-legacy.jsonl`.
