...
- **Still to do:** AR16 step 1 (probe cost on F4, then Q-AR7), AR15c perf run with `UseSourceBytesCache=true` (real machine, quiet), **user GUI check for AR13**: pan + glide, wheel zoom at cursor, Ctrl+wheel navigation, click-to-zoom, Fit after resize and DPI change, skipped-files list opens. Known pre-existing: 5 `Slow` MainWindow integration tests (Actions/FolderSwitch/Explorer) fail identically on master — not caused by this PR, not yet investigated.
