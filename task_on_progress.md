...
- **Mục tiêu:** Tái cấu trúc B + C3 (`docs/refactoring/REFACTOR-PLAN.md`, `REFACTOR-TASKS.md`) kết hợp chẩn đoán hiệu năng WD (`PERF-DIAGNOSIS-PLAN.md`, `PERF-DIAGNOSIS-TASKS.md`). **ĐÃ HOÀN TẤT WAVE 1, WAVE 2, WAVE 3 (T30–T35) VÀ TIẾN TRIỂN MẠNH NHÁNH WC3 VỚI T80, T83, T84, T82 (WicDirectDecoder qua COM Interop WIC trực tiếp), T81 (--decoder-bench CLI benchmark)**.
...
- **Công cụ đo có trong repo:**
  ...
  - `--decoder-bench` (so sánh hiệu năng và bộ nhớ giữa các decoder backend).
...
1. **Lựa chọn task tiếp theo:**
   - **Nhánh WC3 (Thay decoder C3):**
     - **T85** (`TurboJpegDecoder`: libjpeg-turbo C3 decoder).
