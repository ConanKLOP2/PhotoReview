...
- **Measured 2026-09-26 (PERF-STATUS):** AR15c not worse with the source-bytes cache on (new CLI flag `--source-bytes-cache`); AR16 probe ≈ 114 ms per folder open on F4 (≈ 68 % of the old 166 ms first visual) → **Q-AR7 decided (c)** by the user; implemented on `perf/ar16-background-probe` (below).
- **AR16 / Q-AR7 (c) — branch `perf/ar16-background-probe` (pushed, no PR yet):** folder open lists without the per-file open probe and presents the first image; the probe runs in the background, is applied after the Explorer order settles, removes unreadable files (current kept, or advanced like Delete) and still reports them (ADR 0007). GUI check for the user: folder with a locked file → first image at once, then the "skipped" warning.
