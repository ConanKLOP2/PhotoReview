...
- **Cập nhật:** 2026-09-19 | **Branch làm việc:** `refactor/integration` · VERIFY đạt, xUnit 659/659 (Core: 236, Imaging: 144, Integration: 8, Architecture: 5 (+1 skip), App.Tests: 10, Tests.Unit: 256)
...
1. **Lựa chọn task tiếp theo:**
   - **T42b (`UndoService`):** Dịch vụ hoàn tác thao tác tệp di chuyển/sao chép/thùng rác (phụ thuộc T42a - đã sẵn sàng).
   - Hoặc **T43a (`ShortcutRouter`) ∥I:** Định tuyến phím tắt phím đơn/tổ hợp độc lập UI.
   - Hoặc **T43b (`ViewerState`) ∥I:** Quản lý trạng thái khung xem ảnh (zoom, pan, split).
   - Hoặc **T45a (`StatusFormatter`) ∥I:** Định dạng chuỗi thanh trạng thái độc lập UI.
   - Hoặc **T45b (`CompareViewModel`):** ViewModel so sánh ảnh song song.
   - Hoặc **T87:** Tích hợp decoder backend vào ứng dụng chính.
