...
- **T52 DONE:** đã bỏ Folder2Name, chuyển AppConstants → Core `PerformanceOptions`, thêm comment cho `catch { }`, bind các performance defaults từ settings; compatibility marker còn lại được xử lý riêng ở T67. T50a/b, T51, T53a/b, T60, T61 đã DONE.
...
- Biến thể worker S3/F1/warm đã chạy với 0/2/4/8/12 workers, mỗi ô có warm-up + 1 run chính, tất cả pass; summary tại `work/diag/runs/d07-w6-workers-20260919`. Kết quả chính ổn định: presented 144, hit rate 98.6%, final P50 khoảng 7.0–7.6 ms; chưa thấy khác biệt rõ giữa worker counts, cần giữ kết luận là thăm dò vì mỗi ô chỉ 1 run. Disk-cache on/off và PREREAD riêng chưa chạy; cold-os vẫn thủ công.
- Biến thể flag S3/F1/warm đã chạy đủ baseline, `PREREAD=1`, `DISABLE_DISKCACHE=1`, và bật cả hai; mỗi cấu hình warm-up + 1 run chính đều pass. Summary tại `work/diag/runs/d07-w6-flags-20260919`. Final P50 lần lượt khoảng 6.9 ms (baseline), 6.9 ms (PREREAD), 7.4 ms (no disk), 6.9 ms (PREREAD + no disk); hit rate 97.9–98.6%. Đây là thăm dò một run mỗi cấu hình, chưa phải kết luận thống kê cuối.
...
- D12/D13 DONE: `docs/refactoring/diagnosis/REPORT.md` đã được người dùng đọc và xác nhận; `REFACTOR-PLAN.md`, `REFACTOR-TASKS.md`, `PERF-DIAGNOSIS-TASKS.md` đã đồng bộ thứ tự. T67 DONE: xoá marker T46d trong `MainWindow.xaml.cs`, xoá shim chết `ImageSortService.cs`/property source tương ứng, bỏ test RAM đã được T64 thay thế, cập nhật test shutdown theo contract dispose; `SourcePresenceTests` 12/12 PASS. `verify-all.ps1` xác nhận 734 tests PASS (7 Architecture, 305 Core, 201 Imaging, 59 Integration, 161 App), smoke/fault-injection/publish/verify-release PASS. Lần chạy trước có một `UnauthorizedAccessException` nhất thời ở test SessionStore; rerun pass. Tiếp theo là T87 làm setting + fallback với WPF default; T65 BLOCKED đến sau T87/T66.
...
- T65 đang triển khai: đã thêm `SourceBytesCache` LRU/dedup/generation clear/evict, flag `UseSourceBytesCache=false` và capacity 16 GiB; Preview decode, FileHashService và PreloadScheduler dùng cache khi bật. Test cache 2/2, full verify PASS (203 Imaging, 163 App). ThumbnailCache vẫn giữ decode stream cũ do WPF PNG bytes seam chưa ổn định; còn A/B benchmark và quyết định seam thumbnail; chưa bật mặc định.
