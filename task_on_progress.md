...
- **UI feedback Q-R28 (2026-09-26):** arrow keys pan a zoomed image (#119, merged). Integration branch `integration/ui-feedback` (PR open): dark title bar + scrollbar fix, toolbar auto-hide, context menu Open folder/Settings/Click zoom level, title-bar fields, optional Modified date EXIF field, info font size, click-zoom shortcut `2`, glide smoothing `Predict` (60 Hz Dell: judder ~40→12–17/s; 240 Hz: speed jumps 55→11–32/s; 75/144 Hz unit-tested only). **User GUI check pending** for all of these.
- **Known flaky test:** `InfoOverlayFaultTests.SiblingSearchFault_*` can fail under heavy CPU (asserts the pending placeholder after starting the background search — race); passes alone.
