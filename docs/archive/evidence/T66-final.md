...
> **Harness note (AR07 §2):** the S-scenario numbers below were produced by `tools/diag/run-matrix.ps1`, which drives `PhotoReview.Benchmark.Cli -- --perf-session ...` (confirmed in `run-matrix.ps1`), not `BenchmarkEngine`. This is the legacy `--perf-session` graph with no preload — see AR02 F2.
