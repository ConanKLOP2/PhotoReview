...
| `ClickToZoomEnabled` / `ClickZoomPercent` / `ArrowPanStepPercent` / `ArrowKeyNavigatesAtZoomEdge` | `false` / `100` / `10` / `false` | Click (không kéo) chuyển Fit ↔ `ClickZoomPercent` % pixel nguồn (10–800, ADR 0008), neo tại con trỏ. Mũi tên trên ảnh đang zoom chỉ pan, mỗi lần `ArrowPanStepPercent` % viewport (1–100); không chuyển ảnh trừ khi bật `ArrowKeyNavigatesAtZoomEdge`. Menu chuột phải có Fit, Zoom to N % và submenu Zoom levels; Settings ▸ Mouse & zoom có cùng nhóm Zoom. |
