...
**Zoom (option A cho #43):** ngoài Fit, `ViewerState.Zoom` tính theo pixel GỐC — phần tử ảnh có kích thước `OriginalWidth × Zoom / DpiScale` DIP (100 % = 1 pixel nguồn trên 1 pixel thiết bị), không phụ thuộc bitmap đang hiển thị. `ImagePresenter` hiện preview ngay ở đúng kích thước đó, còn `ZoomDetailLoader` gọi `PreviewImageService.DecodeOriginalAsync` (thread riêng, không vào RAM LRU/disk cache) cho ảnh HIỆN TẠI rồi thay `Source` mà không đổi layout/scroll. Chỉ giữ tối đa một original (của ảnh hiện tại); điều hướng sẽ hủy decode chưa chạy, bỏ kết quả trễ (token navigation) và thả original. Về Fit thì dùng lại preview.
