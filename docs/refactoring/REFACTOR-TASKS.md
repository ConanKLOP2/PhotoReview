...
- **Nhật ký:** 2026-09-20 — Follow-up GUI xác nhận Fit lần đầu gần đúng, lần hai mới chuẩn. Source analysis: lần đầu đọc viewport khi scrollbar zoom còn chiếm chỗ; scrollbar biến mất làm viewport đổi mà `ImageScroll.SizeChanged` không bảo đảm chạy; production `ApplyInitialViewMode` còn truyền `(0,0)`. Plan tại `T89-FIT-LAYOUT-PLAN.md`. Đã triển khai pass convergence tối đa 3 lượt, đo lại sau `UpdateLayout`/Render, cập nhật khi ảnh và current presentation đổi; targeted 17/17, full `verify-all.ps1` 741/741, publish + verify-release PASS. Chờ GUI acceptance.
