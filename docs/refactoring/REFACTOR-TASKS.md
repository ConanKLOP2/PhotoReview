...
- **Nhật ký:** 2026-09-20 — Follow-up theo phản hồi GUI: wheel thường (không cần Ctrl) zoom; chuyển scale sang `LayoutTransform` để extent/scroll đồng bộ; Fit truyền `ViewportWidth/Height` thật vào `ResetFit` rồi reset hai offset. Thêm test viewport reset; App 167/167 và `verify-all.ps1` 741/741, publish + verify-release PASS. Chưa đánh dấu DONE vì vẫn cần GUI evidence với Windows Photos behavior trên ảnh dọc/ngang.
