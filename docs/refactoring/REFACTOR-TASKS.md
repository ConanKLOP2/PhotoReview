...
- **Nhật ký:** 2026-09-19 · Codex · `codex/w6-performance` · Đã thêm `SourceBytesCache` LRU theo `(path,length,mtime)`, dedup in-flight, generation clear/evict, cấu hình `UseSourceBytesCache=false` và `SourceBytesCapacityBytes=16 GiB`; `PreviewImageService` và `FileHashService` dùng cache khi flag bật, mặc định không đổi. ThumbnailCache vẫn giữ đường decode cũ vì WPF stream fixture chưa ổn định khi truyền bytes; PreloadScheduler chưa có tầng byte riêng. Test cache 2/2 pass; full `verify-all.ps1` PASS (203 Imaging, 163 App). Chưa đánh dấu DONE: cần hoàn thiện thumbnail/preload seam và A/B sau T66.
