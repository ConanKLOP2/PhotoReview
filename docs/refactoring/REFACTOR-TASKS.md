...
- **Nhật ký:** 2026-09-20 — Follow-up GUI xác nhận Fit lần đầu gần đúng, lần hai mới chuẩn. Đã triển khai Fit convergence tối đa 3 lượt và cập nhật viewport khi ảnh/current presentation đổi. Wheel follow-up được sửa lại: `anchorBefore` tính bằng tọa độ phần tử trước khi đổi sang tọa độ ảnh nguồn, `anchorAfter` dùng tọa độ nguồn sau layout; tránh trộn hai hệ tọa độ gây neo về góc. Targeted 19/19, full xUnit 743/743, smoke, fault injection, publish PASS; verify-release PASS với thư mục publish chỉ định rõ. Chờ GUI acceptance.
